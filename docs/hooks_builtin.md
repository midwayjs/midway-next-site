...
useInject 是对 Midway IoC 依赖注入功能的封装，用于获取注入的实例，等同于 Class 里使用的 `@Inject` 装饰器与 [使用依赖注入](https://www.yuque.com/midwayjs/midway_v2/container) 文档中提到的 `getAsync` 方法。
...
进一步的文档可以参考：[使用依赖注入](https://www.yuque.com/midwayjs/midway_v2/container)。
...
业务配置的使用方式可以参考：[多环境配置](https://www.yuque.com/midwayjs/midway_v2/env_config)
